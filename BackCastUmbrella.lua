local BackCast = {}
BackCast.optionEnable = Menu.AddOptionBool({ "Utility","BackCast" }, "Enabled", "")
...
function BackCast.OnUpdate()
	if used == 1 and Entity.GetVelocity(myHero):Length2DSqr() == 0 then
		...
		--Player.PrepareUnitOrders(play, Enum.UnitOrder.DOTA_UNIT_ORDER_STOP, targ, Vector(0, 0, 0), abil, iss, np, false, true)
